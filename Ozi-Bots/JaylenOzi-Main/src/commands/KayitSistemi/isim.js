...
module.exports = {
  ...
  run: async (client, message, args, perm, prefix) => {
    ...
    {
    message.react(red)
    message.reply({ content:`\`${prefix}isim <@Ozi/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
...
.setFooter({ text: `Üyesinin önceki isimlerine <!isimler @Ozi/ID> komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.` });
